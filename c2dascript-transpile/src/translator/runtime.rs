...
fn heap_address(offset: DaExpr) -> DaExpr {
    ...
    DaExpr::Unsafe(Box::new(call(
        "intptr",
        vec![DaExpr::Unsafe(Box::new(DaExpr::Addr(Box::new(DaExpr::Index(
            Box::new(var(HEAP)),
...
pub fn declarations() -> Vec<DaDecl> {
    ...
    let init_heap = function(
        ...
        vec![DaStmt::Expr(DaExpr::IfThenElse {
            ...
            then: block(vec![
                DaStmt::Expr(call(
                    "reserve",
                    vec![var(HEAP), uint_to_int(DaExpr::ConstUInt(HEAP_RESERVE_BYTES))],
                )),
        ...
        vec![
            ...
            DaStmt::Expr(DaExpr::IfThenElse {
                cond: Box::new(op(
                    ">",
                    var("end"),
                    DaExpr::ConstUInt(HEAP_RESERVE_BYTES),
                )),
                then: block(vec![ret(DaExpr::ConstUInt(0))]),
                elifs: vec![],
                else_: None,
            ...
            DaStmt::Expr(DaExpr::Assign(
                Box::new(DaExpr::Index(Box::new(var(ALLOC_ADDRS)), Box::new(uint_to_int(var("record"))))),
                Box::new(var("start")),
            )),
            DaStmt::Expr(DaExpr::Assign(
                Box::new(DaExpr::Index(Box::new(var(ALLOC_SIZES)), Box::new(uint_to_int(var("record"))))),
                Box::new(var("size")),
            )),
            DaStmt::Expr(DaExpr::Assign(
                Box::new(DaExpr::Index(Box::new(var(ALLOC_LIVE)), Box::new(uint_to_int(var("record"))))),
    ...
    let free = function(
        "c2da_rt_free",
        vec![DaStmt::Param { name: "address".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false }],
        DaType::void(),
        vec![
            DaStmt::Var { name: "i".to_owned(), var_type: uint64.clone(), init: Some(DaExpr::ConstUInt(0)) },
            DaStmt::Expr(DaExpr::While(
                Box::new(op("<", uint_to_int(var("i")), call("length", vec![var(ALLOC_ADDRS)]))),
                block(vec![
                    DaStmt::Expr(DaExpr::IfThenElse {
                        cond: Box::new(op(
                            "&&",
                            DaExpr::Index(Box::new(var(ALLOC_LIVE)), Box::new(uint_to_int(var("i")))),
                            op("==", DaExpr::Index(Box::new(var(ALLOC_ADDRS)), Box::new(uint_to_int(var("i")))), var("address")),
                        )),
                        then: block(vec![
                            DaStmt::Expr(DaExpr::Assign(
                                Box::new(DaExpr::Index(Box::new(var(ALLOC_LIVE)), Box::new(uint_to_int(var("i"))))),
    ...
    let realloc = function(
        "c2da_rt_realloc",
        vec![
            DaStmt::Param { name: "address".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
            DaStmt::Param { name: "size".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
        ],
        uint64.clone(),
        vec![
            DaStmt::Expr(DaExpr::IfThenElse {
            ...
            DaStmt::Expr(DaExpr::IfThenElse {
                cond: Box::new(op("==", var("size"), DaExpr::ConstUInt(0))),
                then: block(vec![DaStmt::Expr(call("c2da_rt_free", vec![var("address")])), ret(DaExpr::ConstUInt(0))]),
                elifs: vec![], else_: None,
            }),
            DaStmt::Var { name: "i".to_owned(), var_type: uint64.clone(), init: Some(DaExpr::ConstUInt(0)) },
            DaStmt::Var { name: "old_size".to_owned(), var_type: uint64.clone(), init: Some(DaExpr::ConstUInt(0)) },
            DaStmt::Var { name: "found".to_owned(), var_type: DaType::bool(), init: Some(DaExpr::ConstBool(false)) },
            DaStmt::Expr(DaExpr::While(
                Box::new(op("<", uint_to_int(var("i")), call("length", vec![var(ALLOC_ADDRS)]))),
                block(vec![
                    DaStmt::Expr(DaExpr::IfThenElse {
                        ...
                        then: block(vec![
                            assign("old_size", DaExpr::Index(Box::new(var(ALLOC_SIZES)), Box::new(uint_to_int(var("i"))))),
            ...
            DaStmt::Expr(DaExpr::IfThenElse {
                ...
                then: block(vec![ret(DaExpr::ConstUInt(0))]), elifs: vec![], else_: None,
            }),
            DaStmt::Var { name: "replacement".to_owned(), var_type: uint64.clone(), init: Some(call("c2da_rt_malloc", vec![var("size")])) },
            DaStmt::Expr(DaExpr::IfThenElse {
            ...
            DaStmt::Expr(call("c2da_rt_memcpy", vec![var("replacement"), var("address"), var("copy_size")])),
    ...
    let memset = function(
        "c2da_rt_memset",
        vec![
            DaStmt::Param { name: "dst".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
            DaStmt::Param { name: "value".to_owned(), param_type: DaType::uint8(), default: None, is_mutable: false },
            DaStmt::Param { name: "count".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
        ],
        uint64.clone(),
        vec![
            DaStmt::Var { name: "i".to_owned(), var_type: uint64.clone(), init: Some(DaExpr::ConstUInt(0)) },
    ...
    let calloc = function(
        ...
        uint64.clone(),
        vec![
            DaStmt::Var { name: "total".to_owned(), var_type: uint64.clone(), init: Some(op("*", var("count"), var("size"))) },
            ...
            DaStmt::Var { name: "address".to_owned(), var_type: uint64.clone(), init: Some(call("c2da_rt_malloc", vec![var("total")])) },
            DaStmt::Expr(DaExpr::IfThenElse {
                cond: Box::new(op("!=", var("address"), DaExpr::ConstUInt(0))),
                then: block(vec![DaStmt::Expr(call(
                    "c2da_rt_memset",
                    vec![var("address"), uint_to_uint8(DaExpr::ConstUInt(0)), var("total")],
                ))]),
                elifs: vec![],
                else_: None,
            }),
    ...
    let memcpy = function(
        ...
        vec![
            DaStmt::Param { name: "dst".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
            DaStmt::Param { name: "src".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
            DaStmt::Param { name: "count".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
        ],
        uint64.clone(),
        vec![
    ...
    let memcmp = function(
        "c2da_rt_memcmp",
        vec![
            DaStmt::Param { name: "left".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
            DaStmt::Param { name: "right".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
            DaStmt::Param { name: "count".to_owned(), param_type: uint64.clone(), default: None, is_mutable: false },
        ],
        DaType::int(),
        vec![
            ...
            DaStmt::Expr(DaExpr::While(
                ...
                block(vec![
                    DaStmt::Expr(DaExpr::IfThenElse {
                        cond: Box::new(op("<", byte_to_uint(raw_byte_at(var("left"), var("i"))), byte_to_uint(raw_byte_at(var("right"), var("i"))))),
        ...
        vec![
            ...
            DaStmt::Expr(DaExpr::IfThenElse {
                ...
                else_: Some(block(vec![
                    ...
                    DaStmt::Var { name: "i".to_owned(), var_type: uint64.clone(), init: Some(var("count")) },
                    DaStmt::Expr(DaExpr::While(
    ...
    vec![
        ...
        DaDecl::Variable(DaVariable { name: ALLOC_ADDRS.to_owned(), var_type: DaType::array(DaType::uint64()), init: None, annotations: vec![] }),
        DaDecl::Variable(DaVariable { name: ALLOC_SIZES.to_owned(), var_type: DaType::array(DaType::uint64()), init: None, annotations: vec![] }),
        DaDecl::Variable(DaVariable { name: ALLOC_LIVE.to_owned(), var_type: bools, init: None, annotations: vec![] }),
        init_heap,
        malloc,
        free,
        realloc,
...
mod tests {
    ...
    fn runtime_is_ast_generated_and_uses_explicit_pointer_to_address_conversion() {
        ...
        assert!(rendered.contains("c2da_rt_memset(address, uint8(0), total)"));
