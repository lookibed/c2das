...
| wasm3 (interpreter core, no WASI) | `tests/manual/wasm3/UPSTREAM.md`: wasm3 `deeaca9ce` (MIT); `fixtures/fib32.wasm`, `fib64.wasm` are upstream's own test vectors | translated, runs under `-jit` only | none yet: `src/all_host.c` (graph + C host over the `.wasm` named by the last argument) translates under `--libc std`; not registered until the interpreter path runs | `fib[n]=` for n in 1,2,5,10,15,20,24 plus `count=7`; native `-O2` and `-O0` and `daslang -jit` agree byte for byte | `docs/followups/translator_gaps_wasm3.md`: measured per mode (aot 1.57×, `-exe` 2.74×, `-jit` 2.83× of C `-O2`); the plain interpreter stops on a daslang-side extern binding error ([lookibed/daScript#3](https://github.com/lookibed/daScript/issues/3)) and AOT needs a struct reorder ([#2](https://github.com/lookibed/daScript/issues/2)), both being avoided translator-side; decisions on the four translator gaps still due |
