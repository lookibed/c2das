...
2. **`[unsafe_deref]` on every translated function.**  `ExprAt`, `ExprPtr2Ref` and field
   ...
   *Revised, 2026-09-23: an option, not the benchmark's default.*  After a conversation
   with the daslang author the owner took `--unsafe-deref` out of the corpus cases'
   `translator_flags`.  The author's reading: it "turns das into C; you could rewrite it on
   pointers and get the same; the question is which unsafe you like".  So the headline and
   every unsuffixed row of `docs/corpus-benchmark.md` now measure the translator's
   defaults (null checks kept), and the flag is a benchmark option: each corpus block
   carries `"optional_translator_flags": {"unsafe_deref": ["--unsafe-deref"]}`, and
   `corpus_matrix.py bench` translates the case a second time with it and reports `daslang
   jit + unsafe_deref`, `daslang exe + unsafe_deref` and `daslang aot + unsafe_deref` rows,
   plus a table under the headline with each option row's change against the same mode
   without it (`docs/corpus-build-recipe.md`, step 1a).  A second reason not to make it a
   default: the annotation has a real miscompile, filed as
   [lookibed/daScript#7](https://github.com/lookibed/daScript/issues/7) — a nested `void`
   function whose only effect is a store through `reinterpret<uint8?>(uint64)` is dropped
   in all modes under `[unsafe_deref]`.  The per-frame hash check guards the corpus rows
   against it; it does not guard an arbitrary translated program.
...
*Revised, 2026-09-23.*  2 is no longer part of the measured default: the corpus cases
declare no `translator_flags`, and `--unsafe-deref` is measured as its own `+
unsafe_deref` rows (see lever 2's revision).  The numbers are regenerated by the matrix
run, not written by hand.

## The next lever: vectorization (daslang author's advice, 2026-09-23)

With `solid_context` in and `unsafe_deref` moved to an option, the daslang author named
the next lever outright: "the key focus is vectorization".  His specifics, each of which
turns into work here:

- **Loops that do not vectorize are the gap.**  "An `int16` loop with `& (N-1)` and an
  `int64` loop do not vectorize and are 8–10× slower."  These are exactly the two probes
  under *daslang-side findings* below: the `int16` ring-index wrap
  ([lookibed/daScript#6](https://github.com/lookibed/daScript/issues/6)) and the `int64`
  multiply/shift loop (8.5× behind `-march=native` C, not yet reduced to a pure repro).
  The next step is to find which of the decoders' hot loops have either shape and check
  the JIT's IR (`--jit-dump`) for a vector body.
- **At least unroll.**  Where a loop cannot be vectorized, unrolling it still pays; which
  of the translated hot loops LLVM leaves rolled is read from the same IR dump.
- **A constexpr macro on fixed inputs.**  Values that are fixed for a whole run (table
  sizes, strides, block shapes) can be folded at compile time by a daslang macro, so the
  loop the JIT sees has constant bounds and can be vectorized or fully unrolled.
- **Profile, then read the IR.**  "Take a profiler and look at where the jit spends time
  and which IR it generates."  The research runs above had no `perf`/`valgrind` and
  attributed by controlled variants; the next round starts from a profile of the jitted
  decode loop and the IR of its top functions.

One fact for that round: the checked-out daslang does not contain upstream #4089, so a
builtin `memcpy` is still a libc call under `-jit` here (lever 5's small-block copies
included).
