...
`tests/manual/real-world-h264bsd-mp4` (h264bsd + minimp4) and
`tests/manual/real-world-plmpeg-stream` (pl_mpeg), in an environment that has only the
...
3. **Iterate to green on the fixture oracles.** Loop: transpile, compile with daslang, fix
   ...
   `tests/manual/real-world-plmpeg-stream/run_end_to_end.sh` with
   `plmpeg_reference.expected`, and `tests/manual/real-world-h264bsd-mp4/src/test_decode.das`
