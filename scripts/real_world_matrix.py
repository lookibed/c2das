#!/usr/bin/env python3
"""Real-world matrix: every real-world canonical case, in C and in every daslang run mode.

Two commands, two committed documents:

  converge  -> docs/real-world-convergence.md
      For each case with a `real_world` block in tests/canonical/cases.json: build
  ...
  bench     -> docs/real-world-benchmark.md
      Same fresh translation, but the benchmark entries (`real_world.bench_c_entry`,
      `real_world.bench_das_entry`), which print the per-frame hashes plus the time
...
daslang run modes ("interop" in the project's wording is the interpreter):
  ...
  aot      daslang -aot <module>.das <module>.cpp     C++ generated ahead of time,
           ...
           scripts/real_world/aot_host.c against the daslang runtime; the host
...
ROOT = runner.ROOT
CONVERGENCE_DOC = ROOT / "docs/real-world-convergence.md"
BENCHMARK_DOC = ROOT / "docs/real-world-benchmark.md"
AOT_HOST = ROOT / "scripts/real_world/aot_host.c"
...
class Prepared:
    ...
    @property
    def real_world(self) -> dict[str, Any]:
        return self.case["real_world"]

    @property
    def bench_das(self) -> Path:
        return self.copied_root / Path(self.real_world["bench_das_entry"])

    @property
    def bench_c(self) -> Path:
        return self.copied_root / Path(self.real_world["bench_c_entry"])
...
def prepare(case: dict[str, Any], daslang: Path) -> Prepared:
    if "real_world" not in case:
        raise MatrixFailure(f"{case['id']}: not a real-world case (no real_world block)")
    env = os.environ.copy()
...
def converge_case(case: dict[str, Any], daslang: Path, keep: bool) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "fixture_bytes": (p.source_root / p.real_world["fixture"]).stat().st_size,
        }
    finally:
        if keep:
...
def render_convergence(results: list[dict[str, Any]], facts: dict[str, str]) -> str:
    out: list[str] = []
    out.append("# Real-world convergence: C reference vs c2das output, per frame, per run mode\n")
    out.append(
        "Generated by `python3 scripts/real_world_matrix.py converge` "
...
def render_benchmark(results: list[dict[str, Any]], facts: dict[str, str], runs: int) -> str:
    out: list[str] = []
    out.append("# Real-world benchmark: C vs c2das output in every daslang run mode\n")
    out.append(
        "Generated by `python3 scripts/real_world_matrix.py bench` "
        f"on {facts['date']} at commit `{facts['commit']}` "
        f"(daslang {facts['daslang']}; {facts['clang']}; {facts['cpu']}; {facts['os']}, kernel {facts['kernel']}).\n"
    )
    out.append(
...
def select_cases(case_id: str | None) -> list[dict[str, Any]]:
    cases = [c for c in runner.load_cases() if "real_world" in c]
    if case_id:
        cases = [c for c in cases if c["id"] == case_id]
        if not cases:
            raise MatrixFailure(f"no real-world case named {case_id}")
    if not cases:
        raise MatrixFailure("no real-world cases registered")
    return cases
