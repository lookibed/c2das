...
DASCHECK_SH: str = r"""#!/usr/bin/env bash
# this file was autogenerated by templates.py
set -e; set -o pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_FILE="$SCRIPT_DIR/$(basename "$0")".log
DASLANG="${DASLANG:-daslang}"

rm -f "$LOG_FILE"

if ! command -v "$DASLANG" >/dev/null 2>&1; then
    echo "daslang not found: $DASLANG" | tee "$LOG_FILE"
    exit 1
fi

mapfile -t files < <(find "$SCRIPT_DIR/repo" -name '*.das' -type f | sort)
file_count="$(printf '%s\n' "${files[@]}" | sed '/^$/d' | wc -l)"
if [[ "$file_count" -eq 0 ]]; then
    echo "no .das files generated under $SCRIPT_DIR/repo" | tee "$LOG_FILE"
    exit 1
fi
...
for file in "${files[@]}"; do
    rel="${file#$SCRIPT_DIR/}"
    if "$DASLANG" "$file" >>"$LOG_FILE" 2>&1; then
...
echo "checked $file_count .das files, failed $failed" | tee -a "$LOG_FILE"
exit "$failed"
"""
...
def autogen_dascheck(conf: str, yaml: dict[str, Any]) -> Generator[Path, None, None]:
    dascheck = yaml.get("dascheck")
    if not isinstance(dascheck, Mapping):
        return
    ag = dascheck.get("autogen")
    if not (ag and isinstance(ag, bool)):
        return

    out_path = os.path.join(os.path.dirname(conf), "dascheck.gen.sh")
    render_script(DASCHECK_SH, out_path, {})
    yield Path(out_path)
...
def autogen(conf: Config) -> Generator[Path, None, None]:
    ...
    for cf, yaml in conf.project_conf.items():
        ...
        yield from autogen_dascheck(cf, yaml)
