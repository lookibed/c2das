...
Manual fixture for c2dascript using the Spider h264bsd + minimp4 case.
...
```powershell
powershell -ExecutionPolicy Bypass -File tests\manual\h264bsd-mp4\run_dascheck.ps1
```
...
```powershell
powershell -ExecutionPolicy Bypass -File tests\manual\h264bsd-mp4\run_dascheck.ps1 -File src\module_all.das
```
...
```text
tests\manual\h264bsd-mp4\compile_commands.json
```
